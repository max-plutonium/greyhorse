...
@pytest.mark.asyncio
async def test_async_local_run(local_root_password):
...
@pytest.mark.asyncio
async def test_async_local_interactive(local_root_password):
    conn = AsyncLocalConnection(sudo_password=local_root_password)

    async with conn.session() as session:
        async with session.create_process(f'{os.getcwd()}/tests/echo.sh', as_bytes=True) as proc:
