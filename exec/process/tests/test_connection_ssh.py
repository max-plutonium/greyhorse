...
@pytest.mark.asyncio
async def test_async_ssh_run(ssh_host, ssh_username, ssh_root_password):
    conn = AsyncSshConnection(ssh_host, username=ssh_username, sudo_password=ssh_root_password)

    async with conn.session() as session:
        res = await session.run(f'echo "123"', shell=True, as_bytes=True)
        assert res.stdout == b'123\n'

        res = await session.run(f'echo "123"', shell=True)
        assert res.stdout == '123'

        res = await session.run(f'pwd', shell=False)
        assert res.stdout == f'/home/{ssh_username}'

        res = await session.sudo(f'whoami', shell=True)
        assert res.stdout == 'root'


@pytest.mark.asyncio
async def test_async_ssh_interactive(ssh_host, ssh_username):
    conn = AsyncSshConnection(ssh_host, username=ssh_username)

    async with conn.session() as session:
        async with session.create_process('/mnt/files/echo.sh', as_bytes=True) as proc:
        ...
        async with session.create_process('/mnt/files/echo.sh') as proc:
            line1 = await proc.read_line()
