from greyhorse_es.config import EngineConfig
from greyhorse_es.containers import ESAsyncContainer, MultipleESAsyncContainer, SingleESAsyncContainer
from greyhorse_es.engine import ESAsyncEngine
from greyhorse_es.resources import ESAsyncResource
from .conf import ES_URI


def test_base_async_container():
    config = EngineConfig(dsn=ES_URI)

    container = ESAsyncContainer()
    engine = container.create_engine('test', config)

    assert isinstance(engine, ESAsyncEngine)
    assert engine.name == 'test'

    resource = container.instance(container=container)
    assert isinstance(resource, ESAsyncResource)

    container.unwire()


def test_single_async_container():
    container = SingleESAsyncContainer()

    container.config.from_dict({
        'name': 'test_single_async',
        'engine_config': {'dsn': ES_URI},
    })

    engine = container.create_engine()

    assert isinstance(engine, ESAsyncEngine)
    assert engine.name == 'test_single_async'
    assert engine is container.create_engine()

    resource = container.instance(container=container)
    assert isinstance(resource, ESAsyncResource)

    container.unwire()


def test_multiple_async_container():
    container = MultipleESAsyncContainer()

    container.config.from_dict({
        'one': {
            'engine_config': {'dsn': ES_URI},
        },
        'two': {
            'engine_config': {'dsn': ES_URI},
        },
    })

    assert container.create_engine('_') is None
    engine1 = container.create_engine('one')
    engine2 = container.create_engine('two')

    assert isinstance(engine1, ESAsyncEngine)
    assert isinstance(engine2, ESAsyncEngine)
    assert engine1 is not engine2
    assert engine1.name == 'one'
    assert engine2.name == 'two'

    resource = container.instance(container=container)
    assert isinstance(resource, ESAsyncResource)

    container.unwire()
